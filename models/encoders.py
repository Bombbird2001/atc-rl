...
class WSSSAPP02Encoder(Module):
    def __init__(self, node_feature_count, d_model, n_head, n_layers, max_seq_length):
        super().__init__()
        ...
        encoder_layer = TransformerEncoderLayer(d_model=d_model, nhead=n_head, dim_feedforward=64, activation='gelu', batch_first=True)
        self.bn1 = BatchNorm1d(max_seq_length)
        self.encoder = TransformerEncoder(encoder_layer, num_layers=n_layers, norm=self.bn1)
