...
class MultiAircraftGNNPolicy(ActorCriticPolicy):
    ...
    def _build_gnn_extractor(self) -> None:
        self.model = WSSSAPP02GINE(self.node_feature_dim, self.edge_feature_dim)
    ...
    def forward(self, obs: th.Tensor, deterministic: bool = False) -> tuple[th.Tensor, th.Tensor, th.Tensor]:
        ...
        action, latent_rep = self.model(x.x, x.edge_index, x.edge_attr)
        action = self.feature_processor.postprocess_data(action)
